 ...
 * eso es exactamente lo que subpandaTM no hace. El precio de esa decisión es que la
...
export class ErrorDeIA extends Error {
     ...
     * @param {string} mensaje En español, para quien lea el código y para
     *   cuando no haya traducción.
     * @param {{reintentable?: boolean, esperarMs?: number, clave?: string}} [datos]
     *   `clave` es cómo se llama este mensaje en la tabla de textos. Va aquí y
     *   no traducido ya porque core/ no sabe en qué idioma está la interfaz —ni
     *   debe saberlo— y porque el mismo error se enseña en tres sitios
     *   distintos. Un error sin clave es uno cuyas palabras las pone el propio
     *   servicio, y esas no hay forma de traducirlas.
     */
