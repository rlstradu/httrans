/**
 * Los servicios de IA con los que Poanda sabe hablar.
 *
 *   gemini      Google
 *   openai      OpenAI
 *   anthropic   Claude
 *   compatible  cualquier servicio que hable el formato de OpenAI, que es lo
 *               que usan Ollama, LM Studio y llama-server, además de media
 *               docena de servicios de pago
 *
 * El último es el que más rendimiento da por línea escrita: como casi todo el
 * sector copió el formato de OpenAI, con un solo cliente se abre la puerta a la
 * IA que corre en tu propio ordenador y a servicios que no existían cuando esto
 * se escribió. Lo único que cambia es la dirección a la que se llama.
 *
...
const PROVEEDORES = {
    ...
    compatible: {
        id: 'compatible',
        nombre: 'Compatible con OpenAI (Ollama, LM Studio, llama-server…)',
        necesitaClave: false,
        // Ollama por defecto: es la que más se usa y la que menos hay que
        // explicar. LM Studio suele estar en el 1234 y llama-server en el 8080.
        urlPorDefecto: 'http://127.0.0.1:11434/v1',
        modeloPorDefecto: '',

        async chat(datosDeLlamada) {
            return PROVEEDORES.openai.chat({ ...datosDeLlamada, clave: datosDeLlamada.clave || 'local' });
        },

        async modelos(datosDeLlamada) {
            return PROVEEDORES.openai.modelos({
                ...datosDeLlamada,
                clave: datosDeLlamada.clave || 'local',
            });
        },
    },
};
...
export async function conversar(config, mensajes, opciones = {}) {
    ...
    if (!proveedor) throw new ErrorDeIA('servicio', 'No hay ningún proveedor de IA elegido.');

    if (proveedor.necesitaClave && !config.clave) {
        throw new ErrorDeIA('clave', 'Falta la clave de API de este servicio.');
    }
    if (!config.modelo) {
        throw new ErrorDeIA('modelo', 'Falta elegir el modelo.');
    }
...
export async function probarConexion(config, opciones = {}) {
    const proveedor = proveedorPorId(config.proveedor);
    if (!proveedor) return { bien: false, modelos: [], mensaje: 'Elige un servicio de IA.' };
    ...
    try {
        ...
        return { bien: true, modelos, mensaje: 'Conexión correcta.' };
    } catch (error) {
        return { bien: false, modelos: [], mensaje: error.message };
    }
}
