...
import { adivinarIdiomas } from './core/idiomas-declarados.js';
...
const RECUERDO = 'poanda_par_idiomas';
 ...
 *
 * Se pregunta siempre, aunque el archivo lo declare, porque una cabecera
 * equivocada es de las cosas más frecuentes que trae un archivo y enterarse al
 * abrirlo cuesta un clic. Lo que el archivo diga sale propuesto, no impuesto.
 *
 ...
 */
export async function preguntarIdiomasAlAbrir({ formato, nombre, contenido }) {
    const delArchivo = adivinarIdiomas({ formato, nombre, contenido });
    const ultimo = ultimoParUsado();

    const elegido = await pedirIdiomas({
        origen: delArchivo.origen || ultimo.origen,
        destino: delArchivo.destino || ultimo.destino,
        aviso: delArchivo.origen || delArchivo.destino ? t('lang_pair_from_file') : '',
    });
    ...
    // Sin guardar: el proyecto de este archivo todavía no existe, y lo crea
    // registrarProyectoAbierto leyendo ya el par del estado.
